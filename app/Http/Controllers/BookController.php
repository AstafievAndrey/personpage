...
//        $this->middleware('auth');
    ...
    /*открываем стр без конкретной главы*/
    public function read($name=null)
    {   
        if(is_null($name)){
            abort(404);
        }
        $book = DB::select('select t1.name, t1.id,
                                   t1.text, t1.translit, 
                                    (select count(*) from books_chapters t2 where t1.id = t2.book_id) count,
                                    (select array_to_string(array(SELECT t3.name FROM genres t3 JOIN books_genres t4 ON t4.genre_id = t3.id where t4.book_id = t1.id),\',\')) genres
                            from books t1
    ...
    /*открываем стр */
    public function readChapter($name,$glava) 
    {
        if(is_null($name)){
            abort(404);
        }
        ...
        if(count($book)!=0){
            $chapters = DB::select('select t2.name,t2.translit,t2.title,t2.desc,t2.keywords
                            ...
                            join chapters t2 on t2.id = t1.chapter_id
                            where book_id = ?',[$book[0]->id]);
            $chapter = DB::select('select t2.name,t2.translit,t2.title,t2.desc,t2.keywords,t2.text
                            from books_chapters t1
                            join chapters t2 on t2.id = t1.chapter_id
                            where book_id = ? and t2.translit = ?',[$book[0]->id,$glava]);
            if(count($chapter)==1){
                return view('book.readChapter',["h1"=>$book[0]->name,"book"=>$book[0],"chapters"=>$chapters,"chapter"=>$chapter[0]]);
            }
        }else{
            abort(404);
        }
